...
// #define DEBUG_TEST_S21_FLOOR

void test_proc_s21_floor(const s21_decimal *psrc, const s21_decimal *ptdst,
                         const int *ptres);

// 1. Tests with correct parameters (return res = 0)

START_TEST(s21_floor_t101) {  // src = +0
  s21_decimal src = {{0, 0, 0, 0}};
  s21_decimal tdst = {{0, 0, 0, 0}};
  int tres = 0;
  test_proc_s21_floor(&src, &tdst, &tres);
}
END_TEST

START_TEST(s21_floor_t102) {  // src = -0
...
START_TEST(s21_floor_t103) {  // src > 0  (scale = 0)
...
START_TEST(s21_floor_t104) {  // src < 0  (scale = 0)
...
START_TEST(s21_floor_t105) {  // src = +0  (scale = 5)
...
START_TEST(s21_floor_t106) {  // src = -0  (scale = 5)
...
START_TEST(s21_floor_t107) {  // src > 0 (scale = 5)
...
START_TEST(s21_floor_t108) {  // src < 0 (scale = 5)
...
START_TEST(s21_floor_t109) {  // src = +MAX_MANTISSA (scale = 1)
  s21_decimal src = {{0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x00010000}};
  s21_decimal tdst = {{0x99999999, 0x99999999, 0x19999999, 0x00000000}};
  // 79228162514264337593543950335 / 10^1 = 19999999 99999999 99999999
  int tres = 0;
  test_proc_s21_floor(&src, &tdst, &tres);
}
END_TEST

START_TEST(s21_floor_t110) {  // src = -MAX_MANTISSA (scale = 1)
...
START_TEST(s21_floor_t111) {  // src => +MAX_MANTISSA,  scale = 28 (MAX)
...
START_TEST(s21_floor_t112) {  // src => -MAX_MANTISSA,  scale = 28 (MAX)
...
START_TEST(s21_floor_t113) {  // src = +1e-28
...
START_TEST(s21_floor_t114) {  // src = -1e-28
  s21_decimal src = {{0x00000001, 0, 0, 0x801C0000}};
  s21_decimal tdst = {{0 + 1, 0, 0, 0x80000000}};
  int tres = 0;
  test_proc_s21_floor(&src, &tdst, &tres);
}
END_TEST
...
START_TEST(s21_floor_t201) {  // test s21_decimal dst == NULL
  s21_decimal src = {{1234567890, 0, 0, 0x00050000}};
  // s21_decimal tdst = {{12345, 0, 0, 0x00000000}};
  int tres = 1;

  // s21_decimal dst = {{0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF}};
  int res = s21_floor(src, NULL);
  ck_assert_int_eq((int)res, (int)tres);
}
END_TEST

START_TEST(s21_floor_t202) {  // test s21_decimal src is incorrect
  s21_decimal src = {{1234567890, 0, 0, 0x00050011}};
  // s21_decimal tdst = {{12345, 0, 0, 0x00000000}};
  int tres = 1;

  s21_decimal dst = {{0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF}};
  int res = s21_floor(src, &dst);
  ck_assert_int_eq((int)res, (int)tres);
}
END_TEST

START_TEST(s21_floor_t203) {  // test s21_decimal scale is incorrect (>28)
...
Suite *get_suite_s21_floor(void) {
  ...
  tcase_add_test(tc, s21_floor_t101);
  tcase_add_test(tc, s21_floor_t102);
  tcase_add_test(tc, s21_floor_t103);
  tcase_add_test(tc, s21_floor_t104);
  tcase_add_test(tc, s21_floor_t105);
  tcase_add_test(tc, s21_floor_t106);
  tcase_add_test(tc, s21_floor_t107);
  tcase_add_test(tc, s21_floor_t108);
  tcase_add_test(tc, s21_floor_t109);
  tcase_add_test(tc, s21_floor_t110);
  tcase_add_test(tc, s21_floor_t111);
  tcase_add_test(tc, s21_floor_t112);
  tcase_add_test(tc, s21_floor_t113);
  tcase_add_test(tc, s21_floor_t114);
  tcase_add_test(tc, s21_floor_t201);
  tcase_add_test(tc, s21_floor_t202);
  tcase_add_test(tc, s21_floor_t203);
  suite_add_tcase(s, tc);

  return s;
}

// common part of the tests for s21_floor func is placed to the
// proc
void test_proc_s21_floor(const s21_decimal *psrc, const s21_decimal *ptdst,
                         const int *ptres) {
#ifdef DEBUG_TEST_S21_FLOOR
